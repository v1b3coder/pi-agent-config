...
if [ -f "$LOCKFILE" ]; then
	...
	if [ -n "$LOCK_PID" ] && kill -0 "$LOCK_PID" 2>/dev/null; then
		...
		echo "CodeSearch: background indexing $(codesearch stats . 2>/dev/null | grep -q 'Indexed:.*✅' && echo 'completed' || echo 'FAILED — run init.sh again to retry')"
...
if [ ! -f "$LOCKFILE" ]; then
	if [ -d "$CS_DIR" ] && codesearch stats . 2>/dev/null | grep -q "Indexed:.*✅"; then
		...
		echo "  Next 'init.sh' call will detect and wait for completion."
		...
		(
			...
			if ! codesearch stats . 2>/dev/null | grep -q "Indexed:.*✅"; then
				echo "[init.sh worker] index not healthy after first pass, forcing full rebuild..." >&2
				codesearch index --force --model "$MODEL" . 2>&1
			fi

			if codesearch stats . 2>/dev/null | grep -q "Indexed:.*✅"; then
				echo "[init.sh worker] indexing completed successfully" >&2
			else
				echo "[init.sh worker] FATAL: index still not healthy after rebuild" >&2
			fi
...
echo "Init complete (CodeSearch may still be indexing in background)"
