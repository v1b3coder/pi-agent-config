---
name: code-explorer
description: "Use this when the user needs to understand, trace, or explore unfamiliar code — especially multi-file flows, dependencies, compound queries (find X and everything connected to it), or architecture questions. Triggers when the user asks where something lives by feature (where do we validate, where is the cache defined), what calls or imports something, how a feature decides or flows end-to-end, what breaks if changed, or any query joining two code concerns with 'and' (find X and trace Y, show me X and its dependents). Also triggers when CodeSearch or CodeGraph tools are mentioned, since this skill manages their setup. This is the right skill when the user sounds lost, asks about dependencies or cross-file references, or wants confidence nothing is hidden before editing. Not for simple file reading, single-symbol lookups, debugging/fixing tasks, or non-search tasks."
---
